import { getJudge0LanguageId, poolBatchResults, submitBatch } from "../lib/judge0.js";
...
export const createProblem = async(req, res) =>{
    try {
        ...
        if (!referenceSolutions || typeof referenceSolutions !== 'object') {
            const error = new ApiError(400, "Invalid referenceSolution provided.");
...
export const getAllProblems = async(req, res) =>{
    try {
        ...
        return res.status(200).json(new ApiResponse(200, "Problem fetched successfully.", problems));
    } catch (err) {
        logger.error(err);
...
export const updateProblemById = async(req, res) =>{
    ...
    try {
        ...
        return res.status(201).json(new ApiResponse(201, "Problem updated successfully."));
    } catch (err) {
        logger.error(err);
